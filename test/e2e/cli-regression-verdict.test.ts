...
describe("PRProof CLI REGRESSION E2E", () => {
  ...
  it("returns REGRESSION when the new test passes on base and fails on HEAD", () => {
...
`,
    ...
    // BASE: only existing normal behavior is covered
    writeFile(
      repo,
...
`,
    ...
    // HEAD: regression introduced.
    // Negative discounts are accepted again.
    writeFile(
      repo,
      "src/discount.ts",
      `
...
`,
    ...
    // HEAD: new test describes behavior that used to work on BASE.
    // PRProof will copy this test to BASE, where it passes.
    // On HEAD it fails because the change introduced a regression.
    writeFile(
      repo,
      "test/discount.test.ts",
      `
...
`,
    ...
    commitAll(repo, "introduce discount regression");
